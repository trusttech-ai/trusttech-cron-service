...
export class RewindMessagesJob {
  private evolutionApi = new HttpClient('https://evolution-api.trusttech.space');
      ...
      if (numbersToRewind.length) {
        numbersToRewind.forEach(async (item) => {
            ...
            if (diffDays > 0 && diffDays <= 5) {
              ...
              await this.evolutionApi.post('/message/sendText/trusttech', body, { apiKey: process.env.EVOLUTION_API_KEY });
              
              console.log('[RewindMessagesJob] Executed successfully to number: ', item.user_number, ', message number: ', diffDays);
              return;
            }
            ...
            if (diffDays > 5) {
              ...
              console.log('[RewindMessagesJob] Executed successfully to remove number: ', item.user_number);
              return;
            }
